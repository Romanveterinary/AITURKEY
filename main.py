...
def main(page: ft.Page):
    ...
    try:
        ...
        dd_archive_batch = ft.Dropdown(label="Оберіть партію", expand=True) # Повернули для архіву
        ...
        report_screen = ft.Container(
            content=ft.ListView([
                ...
                ft.Row([dd_litter, dd_droppings]), # Додано сюди
                tf_notes,
        ...
        def on_file_picked(e):
            if e.files and len(e.files) > 0:
                ...
                page.snack_bar = ft.SnackBar(ft.Text("✅ Фото прикріплено! Напишіть текст."), bgcolor="green")
        ...
        # ==========================================
        # ЕКРАН 3: АРХІВ (ПОВЕРНУТО!)
        # ==========================================
        def generate_final_report(e):
            b_name = dd_archive_batch.value
            if not b_name: return
            page.snack_bar = ft.SnackBar(ft.Text("⏳ ШІ формує бізнес-звіт...")); page.snack_bar.open = True; page.update()
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"✅ ЗГЕНЕРОВАНО В АРХІВ!"), bgcolor="green"); page.snack_bar.open = True; page.update()
        ...
        # --- НАШІ ВЛАСНІ БРОНЕБІЙНІ ВКЛАДКИ (ТЕПЕР ЇХ ТРИ) ---
        ...
        main_view = ft.Column([
            ...
            report_screen,
            chat_screen,
            archive_screen # Додано екран архіву
